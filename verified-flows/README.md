...
### 工具 — [`../probe-minbuy.js`](../probe-minbuy.js)(起购金额探针)

**用途**: 扫描全部持仓基金,逐只进买入页读「最低买入 X.XX 元」,筛出 1 元起购的基金。只读,不下单。

**状态**: ✅ 已跑通(2026-07-18,42 只全检出 0 未命中;据此选出 7 只 1 元基金,main.js「真实测试(1元)」7/7 真实成交)。

**关键沉淀**:
1. **起购金额在买入页(不在详情页)**,以 text 形式 `最低买入X.XX元`(部分带「,买入无限额」)。详情页无起购信息,必须点「买入」进买入页读。
2. **dump 要抓 text+desc+id**(不只 text())——借 02 的 `dumpEils/dumpFail` 思路;信息常在非 text 字段。
3. **检测双通道**:关键词(起购/最低买入/最低申购/…) + 「金额起」精确通道(`X元起`/`¥X起`/`起X元`,不误中「最近7天/一起来赚」)。
4. **容错与 02 不同**:批量探针改成「单只失败 dump+继续」+ 回持有页允许 launchPackage 重导航,不因一只卡死终止全部(02 是确定性流程验证,失败即 exit)。
5. **前置**:支付宝停在「理财-基金-持有」页;`MAX_FUNDS` 控制数量(先 3 小测再 999 全集)。

**复用要点**: 复制 02 的工具函数集 + `dumpElems()`(返回元素数组而非打日志)+ `detectMin()` 双通道检测,改主循环即可。
