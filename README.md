...
- **运行历史**：每次策略运行保存为独立批次，可查看完成/暂停/终止/异常状态、成交汇总和逐笔买卖明细（本地保留最近 100 次）。
- **暂停与续跑**：策略进度浮窗可在当前单笔安全结束后暂停或终止；下次点运行会提示继续未完成批次，并从下一笔未执行操作续跑。
...
- 批次计划与执行游标逐笔落盘；异常退出时，正在提交且结果不确定的一笔会标记为“待核对”并跳过续跑，避免重复下单。
